...
public class Dog extends Pet{
	...
	public Dog(boolean x,boolean y,boolean z) throws IOException {
		if(!x) {
			System.out.println(""
					+"请给宠物取个名字吧:\n");
			...
			while(hv<60||hv>100) {
				System.out.println("狗狗的初始健康值应在60-100之间！"
						+ "请重新输入：");
			...
			try {
				...
				+Attributes.getName()+"'and host='"+Attributes.getHost()+"'";
	...
	public void confession(boolean k) {
		...
		+".\n我现在有"+getPS()+"的体力.");
		if(k) {
			System.out.println("快与你的狗狗互动吧！");
		}
	}
	@SuppressWarnings("resource")
	public void menu(boolean y,boolean z) throws IOException {
		while(true) {
			System.out.println("1.喂食\t2.抛球\t3.看病\t4.信息显示\t5.退出");
			...
			if(x.equals("1")) {
				...
				if(a==0) {
					...
					System.out.println("狗狗与你的亲密度为："+getIntimacy());
			...
			if(x.equals("2")) {
				int b=game();
				if(b==2) {
					System.out.println("狗狗生病了，没办法和你玩了~~");
				}
				if(b==1) {
					System.out.println("狗狗已经精疲力劲了~~快让他休息吧");
				}
				if(b==0) {
					System.out.println("狗狗玩的很愉快！！");
					System.out.println("狗狗与你的亲密度为："+getIntimacy());
					System.out.println("健康值为："+getHealth());
					System.out.println("体力为"+getPS());
				}
			...
			if(x.equals("3")) {
				boolean cor=treat();
				if(cor) {
					System.out.println("狗狗身体又好起来了！！！"
							+ "体力充沛饱满,但亲密度下滑了，"
							+ "所以赶紧带它去玩吧！！！");
				}
				else {
					System.out.println("狗狗身体无大碍哟！");
				}
			}
			if(x.equals("4")) {
				boolean k=true;
			...
			if(getPS()>=60&&getPS()<=70||getHealth()>=60&&getHealth()<=70) {
				System.out.println("PS:狗狗的状态有些低迷，"
						+ "注意不要让它生病哦~~");
			}
		}
	}
