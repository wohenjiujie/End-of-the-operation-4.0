...
public class Martes extends Pet{
	static int origin;
	static int change;
	@SuppressWarnings("resource")
	public Martes(boolean x,boolean y,boolean z) throws IOException {
		if(!x) {
			System.out.println(""
					+"请给宠物取个名字吧:\n");
	...
	public void confession(boolean k) {
		...
		+".\n我现在有"+getPS()+"的体力.");
		if(k) {
			System.out.println("快与你的雪貂互动吧！");
		}
	}
	@SuppressWarnings("resource")
	public void menu(boolean y,boolean z) throws IOException {
		while(true) {
			System.out.println("1.喂食\t2.洗澡\t3.看病\t4.信息显示\t5.退出");
			...
			if(x.equals("1")) {
				...
				if(a==0) {
					...
					System.out.println("雪貂与你的亲密度为："+getIntimacy());
			...
			if(x.equals("2")) {
				int b=game();
				if(b==2) {
					System.out.println("雪貂生病了，没办法和你玩了~~");
				}
				if(b==1) {
					System.out.println("雪貂已经精疲力劲了~~快让他休息吧");
				}
				if(b==0) {
					System.out.println("雪貂玩的很愉快！！");
					System.out.println("雪貂与你的亲密度为："+getIntimacy());
					System.out.println("健康值为："+getHealth());
					System.out.println("体力为"+getPS());
				}
			...
			if(x.equals("3")) {
				boolean cor=treat();
				if(cor) {
					System.out.println("雪貂身体又好起来了！！！"
							+ "体力充沛饱满,但亲密度下滑了，"
							+ "所以赶紧带它去玩吧！！！");
				}
				else {
					System.out.println("雪貂身体无大碍哟！");
				}
			}
			if(x.equals("4")) {
				boolean k=true;
			...
			if(getPS()>=60&&getPS()<=70||getHealth()>=60&&getHealth()<=70) {
				System.out.println("PS:雪貂的状态有些低迷，"
						+ "注意不要让它生病哦~~");
			}
		}
	}
